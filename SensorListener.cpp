    ...
    while ((num_sensors = listener->mSensorEventQueue->read(sen_events, 8)) > 0) {
        for (int i = 0; i < num_sensors; i++) {
            if (sen_events[i].type == Sensor::TYPE_ACCELEROMETER) {
                ...
                //CAMHAL_LOGVB(" azimuth = %f pitch = %f roll = %f", sen_events[i].vector.azimuth, sen_events[i].vector.pitch, sen_events[i].vector.roll);
...
status_t SensorListener::initialize() {
    ...
    SensorManager& mgr(SensorManager::getInstanceForPackage(android::String16("Activity Recognition")));
